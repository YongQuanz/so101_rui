...
export class HardwareController {
  ...
  async syncState() {
    ...
    try {
      ...
      const component = (res.component || []).find(c => c.name === CONFIG.hardwareComponentName);
      if (!component) {
        this.logger.log(`'${CONFIG.hardwareComponentName}' not found — is the hardware interface running?`, 'warn');
        return;
      }
    ...
    try {
      await this.ros.callService(
        ...
        'controller_manager_msgs/srv/SetHardwareComponentState',
        {
          ...
          target_state: { id: enable ? 3 : 2, label: enable ? 'active' : 'inactive' },
        }
      );
      this.logger.log('Torque ' + (enable ? 'enabled.' : 'disabled.'), enable ? 'ok' : 'warn');
